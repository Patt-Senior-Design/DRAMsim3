...
void RandomCPU::ClockTick() {
    ...
    if (get_next_) {
        memory_system_.AddTransaction(0, last_addr_, last_write_);
    }
    clk_++;
    return;
}
...
void StreamCPU::ClockTick() {
    ...
    if (!inserted_a_ &&
        ...
        memory_system_.AddTransaction(0, addr_a_ + offset_, false);
        inserted_a_ = true;
    }
    if (!inserted_b_ &&
        memory_system_.WillAcceptTransaction(addr_b_ + offset_, false)) {
        memory_system_.AddTransaction(0, addr_b_ + offset_, false);
        inserted_b_ = true;
    }
    if (!inserted_c_ &&
        memory_system_.WillAcceptTransaction(addr_c_ + offset_, true)) {
        memory_system_.AddTransaction(0, addr_c_ + offset_, true);
        inserted_c_ = true;
    }
    ...
    if (!trace_file_.eof()) {
        ...
        if (trans_.added_cycle <= clk_) {
            get_next_ = memory_system_.WillAcceptTransaction(trans_.addr,
                                                             trans_.is_write);
            if (get_next_) {
                memory_system_.AddTransaction(0, trans_.addr, trans_.is_write);
            }
        }
    }
    clk_++;
